...
var SERVER_URL = '',
    All_Tutorials_ID='267452350330424',
    wit_token = 'WIT_TOKEN',
    Greetings = ["Hello to you :D", "Ahoy ^_^", "Hi, I hope you're having a nice day :'D","Howdy! :D","Hi, I'll assest you with any math problem ^_^", "Hay, Let's solve some math stuff B|","Hi,glad to talk to you :D","Oi ^_^ ","Hay :D"]
const BotDescription = "Hello Name, I'm MathHook a bot that gives you a step by step solution for any math problem through tunneling your problem to symbolab website that provides that service, then I send you the solution as an image attachment, all rights are reserved to Symbolab Inc.";
//Make an ArrayList of users (just for now) , But then it'll be a database
var Users = [];
var me = {
...
function getCurrentUser(event) {
    ...
    for (var i = 0; i < Users.length; i++) {
        if (Users[i].id == CurrentUser.id) {
            console.log("The User exists , We'll take the payload");
            Exists = true;
            CurrentUser.payload = Users[i].payload;
            break;
        }
    }
    if (!Exists) {
...
function SolveFor(event, payload) {
    console.log("Going to Solve " + payload + "and event " + event.message);
    var url = "";
    var senderID = event.sender.id;
    var messageText = event.message.text;
    ...
    var graphName = 'public/Solutions/' + senderID + '_Graph_Solution.png';
    sendTypingOn(senderID);
    surfSymbolab(url, solutionName, graphName, senderID, query);
...
function receivedTextMessage(event, CurrentUser) {
    ...
    if (messageText) {
        ...
        if (expressions['ALPHA'].test(messageText)) {
            if (messageText == "detach") {
                var DetachedPayload = null;
                var DetachingBool = false;
                for (var i = 0; i < Users.length; i++) {
                    if ((Users[i].id == senderID)) {
                        DetachedPayload = Users[i].payload;
                        Users[i].payload = "";
                        DetachingBool = true;
                    }
                }
                if (DetachingBool) {
                    sendTextMessage(senderID, "Successfully detach from " + DetachedPayload + " any query now will be a step by step solution");
                }
                ...
                callWitAI(messageText.toString(), function(error, entities) {
                    if (error) {
                        ...
                        if (entities.intent) {
                            var intent = entities.intent[0].value;
                            if (intent == "Greeting") {
                                var rNo = Math.floor(Math.random() * (Greetings.length - 1));
                                console.log(rNo);
                                console.log("The user is %s", intent);
                                sendTextMessage(senderID, Greetings[rNo]);
                            }
                            else if (intent == "Identification") {
                                sendTextMessage(senderID, "I'm MathHook a bot that gives step by step solutions for math problems and also Math courses and tutorials");
                            }
                            else if (intent == "Tutorials" && (sq.length > 0)) {
                                console.log("The user wants %s", intent);
                                console.log("And the tutorials are on %s", sq);
                                sendTextMessage(senderID, "I'm going to search for courses for " + sq[0]);
                                getTutorials(senderID, sq);
                                //sendTextMessage(senderID, "The user meant "+intent);
                            }
                            else if (intent == "Examples" && (sq.length > 0)) {
                                console.log("The user wants %s", intent);
                                console.log("And the tutorials are on %s", sq);
                                sendTextMessage(senderID, "I'm going to search for examples for " + sq[0]);
                                searchPayload(sq, function (error, Examples) {
                                    ...
                                    getExamples(senderID, Examples[0].payload);
                                });
                            }
                            else if ((intent == "Examples" || intent == "Tutorials") && (sq.length == 0)) {
                                sendTextMessage(senderID, "Sorry I couldn't catch the thing you want " + intent + " for :( , you could try again with simplier way");
                            }
                            else {
                                sendTextMessage(senderID, "Sorry, but I didn't understand what you want");
                                sendTypingOff(senderID);
                            }
                        }
                    }
                ...
                /*
                 sendTextMessage(senderID, "I'm Programmed to Query Mathematical Problems Only , you can choose a class from here");
                 sendMainGenericMessage(senderID, "Data/Main.json");
                 */
            }
        }
...
function receivedQuickReplyMessage(event, CurrentUser) {
    var Linked = false;
    var Quick_Reply = JSON.stringify(event.message.quick_reply);
    var senderID = event.sender.id;
    console.log(Quick_Reply);
    Quick_Reply = Quick_Reply.replace('{"payload":"', '');
    var payload = Quick_Reply.replace('"}', '');
    console.log("Quick Reply is " + event.message.quick_reply);
    var UserFound = false;
    for (var i = 0; i < Users.length; i++) {
        if (Users[i].id == event.sender.id) {
            UserFound = true;
            Users[i].payload = payload;
        }
    }
    if (!UserFound) {
        Users.push(CurrentUser);
    ...
    for (var j = 0; j < Users.length; j++) {
        if (Users[j].id == senderID) {
            console.log(Users[j].payload + "is Payload");
            Users[j].payload = payload;
            Linked = true;
            console.log(Users[j].payload + " is in session");
        }
    }
    if (Linked) {
        sendTextMessage(senderID, "All is Good , Now you're Linked to " + payload.replace("-", " ") + " any problem you query will be under that calculator, if you wish to detach, choose a different method or simply type detach. And if you wish to examples of the calculator you're hooked too and how expressions are type simply type Examples");
    }
}
//Surf Symbolab and Take ScreenShot of Solution and Graph
function surfSymbolab(url, solutionName, graphName, senderID, query) {
    var horseman = new Horseman();
    console.log(url);
    horseman.userAgent('default')
        .viewport(2500, 3000)
        ...
        .then(function() {
            ...
            if (fs.existsSync(solutionName)) {
                ...
                solutionUrl = global.SERVER_URL + '/' + senderID + '_Solution.png';
                console.log(solutionUrl);
                sendTextMessage(senderID, "Found The Solution you need ^_^ ");
                setTimeout(function() {
                    sendImageMessage(senderID, solutionUrl, solutionName);
                }, 500)
            }
...
//Send Examples to The User of Payload
function getExamples(recipientId, payload) {
    console.log(payload);
    var Existance = ["", false, false, false];
    var FileLocation = "public/Solutions/" + recipientId.toString() + "_Example_";
    var Texts = [];
    var Links = [];
    var url = "https://www.symbolab.com/solver/" + payload;
    var horseman = new Horseman();
    horseman.userAgent('default')
        ...
        .then(function() {
            //var l=0;
            sendTextMessage(recipientId, "Here are Examples on " + ((payload == "") ? "General Step by Step Calculator" : payload));
            var x = 1;
            for (var i = 1; i < 4; i++) {
                if (Existance[i]) {
                    setTimeout(function(i) {
                        sendTextMessage(recipientId, Texts[i].replace(/\\:/g, ' ').replace(/\$/g, ''));
                    }, x * 1000, i);
                    setTimeout(function(i) {
                        var ImageUrl = global.SERVER_URL + '/' + recipientId.toString() + "_Example_" + i + ".png";
                        sendImageMessage(recipientId, ImageUrl, FileLocation + i + ".png");
                    }, (x + 1) * 1000, i);
                    x += 4;
                }
            }
        ...
        persistent_menu: [{
            ...
            call_to_actions: [{
                ...
                call_to_actions: [{
                    ...
                    type: "postback",
                    payload: "Bot Description"
                }, {
                    title: "Typing Expressions",
                    type: "postback",
                    payload: "Typing Expressions"
                }, {
                    title: "Maths Sections",
                    type: "postback",
                    payload: "Maths Sections"
                },{
                    type: "web_url",
                    title: "Symbolab Website",
            ...
            },
                {
                    ...
                    url: "https://www.m.me/Mashhour2much"
                },
                {
                    type: "web_url",
                    title: "Send Feedback",
                    url: "https://goo.gl/SG2PpP"
                }]
        }]
    };
    callSendAPIGetStarted(messageData);
...
function setGreetingText() {
    var messageData = {
        greeting: [{
            locale: "default",
            text: "Oi {{user_first_name}}! Let's Do Some Math Stuff :D"
        }]
    };
    callSendAPIGetStarted(messageData);
}
//Send For the User How type the Mathematical Expressions
function sendHowToTypeExpressions(recipientId) {
    var pdfMessage = {
        Message: "Secondly a pdf that has the most important Expressions and how to type them",
        id: 266455673763425
    };
    var firstOpening = "Typing the mathematical expression in messenger is quite simple when you get the handle of it. It's to be in latex. With this symbols show in pictures and videos you'll know how type the most complicated equations on messenger";
    var videosLinks = [{
        Message: "For solving Integrals",
        Link: "https://goo.gl/IdzoHD",
        id: "262793487462977"
    }, {
        Message: "For solving Matrices",
        Link: "https://goo.gl/DyHO45",
    ...
    var videoMessage = "First of all You can watch these videos for Examples";
    var ImagesMessage = "And at last few images on the most important symbols";
    setTimeout(function() {
        sendTextMessage(recipientId, firstOpening);
    }, 1000);
    setTimeout(function() {
...
function sendImageMessage(recipientId, ImageUrl, Image) {
    console.log('Here and the Url is ' + ImageUrl);
    var messageData = {
        recipient: {
            id: recipientId
    ...
    callSendAPIforImages(messageData, Image);
}

function sendVideoMessage(recipientId, LinkUrl) {
    var messageData = {
        recipient: {
            id: recipientId
        },
...
function receivedPostback(event) {
    ...
    if ((payload.indexOf('More')) > -1 && (payload.indexOf('Quick Reply') <= -1)) {
        ...
        if (Linked) {
            sendTextMessage(senderID, "All is Good , Now you're Hooked to " + payload.replace("-", " ") + " any problem you query will be under that calculator, if you wish to detach, choose a different method or simply type detach");
        }
    } else {
        switch (payload) {
            ...
            case 'Get Started':
                ...
                break;
            case 'Typing Expressions':
                sendHowToTypeExpressions(senderID);
                break;
            case 'Maths Sections':
                sendMainGenericMessage(senderID, 'Data/Main.json');
                break;
...
function getUserName(recipientId) {
    request({
        ...
        if (!error && res.statusCode == 200) {
            var body = JSON.parse(res.body);
            var UserName = body.first_name;
            sendTextMessage(recipientId, BotDescription.replace('Name', UserName));
        } else {
            //console.error("Unable to send message Here.");
            //console.error(res.body.error);
            // console.error(error);
...
function callSendAPIforImages(messageData, Image) {
    request({
        uri: 'https://graph.facebook.com/v2.8/me/messages',
        qs: {
            access_token: PAGE_ACCESS_TOKEN
...
function callWitAI(query, callback) {
    query = encodeURIComponent(query);
    request({
        uri: "https://api.wit.ai/message?v=20170307&q=" + query,
        qs: {
            access_token: wit_token
        },
        method: 'GET'
...
function getTutorials(senderID, Keyword) {
    ...
    var KeywordFile='';
    var FoundElements = [];
    var FilesNames = ["Algebra (KA)", "Algebra Basics", "Algebra I", "Algebra II", "Calculus", "Geometry (KA)", "Pi Tutorial", "Pre Calculus", "Prime Numbers", "Trigonomtry", "Statistics", "Fourier Series",
        "Algebra II",
        "Algebra I",
        "Algebra Basics",
        "Algebra (KA)",
        ...
        "Linear Algebra (pjmt)",
        "Pre-algebra (KA)",
        "Pre-algebra and Arithmetic (pjmt)",
        "Calculus - Semester I (pjmt)",
        "Calculus - Semester II (pjmt)",
        "Calculus - Semester III (pjmt)",
        "Calculus II",
        "Precalculus (KA)",
        "Geometry",
        "Geometry Basics",
    ...
    if (ResultsCount > 0) {
        setTimeout(function(){
            sendTextMessage(senderID, "Found " + ResultsCount + " results for " + KeywordFile + ".");
        },500);
        var TutorialsFile = 'public/Solutions/' + KeywordFile.replace(/ /g, '_') + '_' + 'Tutorials.csv';
        var TutorialsUrl = global.SERVER_URL + '/' + KeywordFile.replace(/ /g, '_') + '_' + 'Tutorials.csv';
        try {
            var writer = csvWriter({
                headers: ["Section", "Name", "Youtube Link"]
            });
...
//getExamples(1424957394212528,"system-of-inequalities-calculator");
//TestTutorials("Tutorials on Limits.");
/*
 .evaluate(function()
 {
 var $selector=$('ul.solution-examples');
 //  return $selector.children().text();
 // var $allElements=$('li');
 // return $selector.find('li').each;
 var Texts=[];
 var Links=[];
 $('li', 'ul.solution-examples').each(function () {
 Texts.push($(this).children().children().text());
 Links.push($(this).children().attr("href")); //log every element found to console output
 });
 return {Texts:Texts,Links:Links};
 })
 */
/*
 var videoMessage="First of all You can watch these videos for Examples";
 var IntegralsMessage="For solving Integrals";
 var IntegralsLink="https://goo.gl/IdzoHD";
 var MatricesMessage="For solving Matrices";
 var MatricesLink="";
 var SeriesMessage="For solving Series and Limits";
 var SeriesLink="";
 sendTextMessage(recipientId,videoMessage);
 sendTextMessage(recipientId,IntegralsMessage);
 sendVideoMessage(recipientId,IntegralsLink);
 sendTextMessage(recipientId,IntegralsMessage);
 sendVideoMessage(recipientId,IntegralsLink);
 sendTextMessage(recipientId,IntegralsMessage);
 sendVideoMessage(recipientId,IntegralsLink);
 */
/* // try {
 if (entities.search_query[0] != undefined) {
 sq = entities.search_query[0].value;
 if (sq.match(/Examples/i)) {
 try {
 sq = entities.search_query[1].value;
 } catch (e) {
 console.log(e);
 }
 }
